    ...
    <script>
        ...
        $().ready(function(){
            ...
            $.getJSON("getallcomplaintslog.php",function(data){
                ...
                var trHTML = '';
                $.each(data, function (i, item) {
                    trHTML+='<tr><td>'+item.complaintId+'</td><td>'+item.createdOn+'</td><td>'+item.name+'</td><td>'+item.address+'</td><td>'+item.category+'</td><td>'+item.status+'</td><td>'+item.closuretime+'</td><td>'+item.resource+'</td></tr>';
                });
            ...
            $.getJSON("getallpaniclog.php",function(data){
                ...
                var trHTML = '';
                $.each(data, function (i, item) {
                    trHTML+='<tr><td>'+item.panicid+'</td><td>'+item.requesttime+'</td><td>'+item.name+'</td><td>'+item.address+'</td><td>'+item.panictype+'</td><td>'+item.respondername+'</td><td>'+item.closureauthority+'</td><td>'+item.closuretime+'</td><td>'+item.status+'</td><td><a target="_blank" href="http://localhost/bcp/demo/getallpanicattachements.php?panicid='+item.panicid+'">Images</a></td></tr>';
                });
            ...
            $('#createNewCampaignForm').ajaxForm({
                ...
                complete:function(response){    
                    ...
                    $.getJSON("getallcampaigns.php",function(data){
                        ...
                        var trHTML = '';
                        $.each(data, function (i, item) {
                            trHTML+='<tr><td>'+item.campaignName+'</td><td>'+item.moderatedOn+'</td><td>'+item.sentto+'</td><td>'+item.totalseen+'</td><td>'+item.totalackd+'</td><td>'+item.totalreported+'</td></tr>';
                        });
            ...
            window.setInterval(function(){
                ...
                $.getJSON("securitypanicweb.php",function(data){
                    ...
                    $.each(data, function (i, item) {
                        ...
                        $("#panic_time").text(item.requesttime);
                    });
            ...
            $('#reportsmodel').on('show.bs.modal', function (e) { //Modal Event
                ...
                $.getJSON("getcampaignreports.php?id="+id, function(data){
                    ...
                    var trHTML = '';
                    $.each(data, function (i, item) {
                            trHTML += '<tr><td>'+item.reportedOn+'</td><td>'+item.name+'</td><td>'+item.contact+'</td><td>'+item.address+'</td><td>'+item.reportmsg+'</td></tr>';
                    });
    ...
    <div class="row">
        <div class="container" style="background-color:#e74c3c ">
            <div style="padding: 10px; box-sizing: border-box;text-align: center; color:#fff">
                ...
                <label style="font-size: large;" id="panic_name">NAME</label> <br>
                <label style="font-size: large;" id="panic_address">Address</label> <br>
...
<section>
    <!-- centered pills -->
    <div class="row" style="background-color: #333;padding:5px">
        <div class="row" style="margin-left: 20px;margin-right: 20px;">
            ...
            <div class="row" style="margin:0px auto">
                <ul class="nav nav-pills " id="myTab">
                    ...
                    <li ><a data-toggle="pill" href="#tab2">SECURITY</a></li>
                    <li ><a data-toggle="pill" href="#tab3">COMPLAINTS</a></li>
